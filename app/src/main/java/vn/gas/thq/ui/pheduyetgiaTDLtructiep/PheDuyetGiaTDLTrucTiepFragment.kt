...
import vn.gas.thq.ui.pheduyetgia.RequestApproveAdapter
...
class PheDuyetGiaTDLTrucTiepFragment : BaseFragment(), RequestApproveAdapter.ItemClickListener {
    private lateinit var viewModel: PheDuyetGiaTDLTrucTiepViewModel
    ...
    companion object {
        @JvmStatic
        fun newInstance(): PheDuyetGiaTDLTrucTiepFragment {
            val args = Bundle()

            val fragment = PheDuyetGiaTDLTrucTiepFragment()
    ...
    override fun setupViewModel() {
        viewModel =
            ViewModelProviders.of(this,
                ...
                .get(PheDuyetGiaTDLTrucTiepViewModel::class.java)
    }

    override fun initView() {
        tvTitle.text = "Phê duyệt yêu cầu bán trực tiếp"
...
//        edtType.setOnClickListener(this::onChooseType)
    ...
    private fun onSearchData(view: View) {
        ...
        if (AppDateUtils.validateEndDateGreaterorEqualThanStartDate(fromDate, endDate)) {
            viewModel.onSearchRetailTDL(status, staffCode, fromDate, endDate, 0)
        } else {
    ...
    private fun setEndLessScrollListener() {
        ...
        rvRequestApprove.addOnScrollListener(object :
            ...
            override fun onLoadMore(page: Int, totalItemsCount: Int, view: RecyclerView?) {
                ...
                viewModel.onSearchRetailTDL(status, staffCode, fromDate, endDate, page)
            }
        })
    }
...
//        showMess("Bạn không có quyền thực hiện với yêu cầu này")
    ...
    private fun showDiglogDetailRetail() {
        ...
        val dialogView: View = inflater.inflate(R.layout.layout_dialog_item_duyet_gia_tdl, null)
...
//        }
    ...
    private fun showDiglogDetailRetailV2() {
        val builder = context?.let { AlertDialog.Builder(it, R.style.AlertDialogNoBG) }
        val inflater = this.layoutInflater
        val dialogView: View = inflater.inflate(R.layout.layout_dialog_item_direct_retail, null)
        builder?.setView(dialogView)
        val linearLayoutManager = LinearLayoutManager(context, LinearLayoutManager.VERTICAL, false)
//        adapterDetailYCXK = DetailItemProduct4Adapter(mDetailYCXKData!!.item)
        ...
        val linearAccept: LinearLayout = dialogView.findViewById(R.id.linearAccept)
        val llWrapGhiChu: LinearLayout = dialogView.findViewById(R.id.llWrapGhiChu)
        val btnTuChoi: Button = dialogView.findViewById(R.id.btnTuChoi)
        val btnDongY: Button = dialogView.findViewById(R.id.btnDongY)
...
//            rvProductDialog.adapter = adapterDetailYCXK
            ...
            if (mDetailRetailData?.approveStatus != "555" && mDetailRetailData?.approveStatus != "444") {
                llWrapGhiChu.visibility = View.VISIBLE
                linearAccept.visibility = View.VISIBLE
            } else {
                llWrapGhiChu.visibility = View.GONE
...
//        showMess(mList[position].approve_status)
        viewModel.detailTDL(orderId.toString())
    }
}